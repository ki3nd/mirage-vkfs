...
This package is a standalone Mirage plugin. It does not require Dify support to
be merged into `mirage-ai`; import `DifyResource` from `mirage_vkfs` and pass it
to `mirage.workspace.Workspace`.

```python
from mirage.workspace import Workspace
from mirage_vkfs.resource.dify import DifyResource
```
